...
def matrix_inverse_root(
    ...
    def matrix_inverse_root_chol(
        ...
        #root=4
        #print("EPS", epsilon)
        #return matrix_inverse_root_newton(A,root,epsilon)
        A+=epsilon*torch.eye(A.shape[0])
        C,info=torch.linalg.cholesky_ex(A) #A=C@C.T, approx. C=A^-1/2
        if info==0:
            print("CHOL: ", C)
            return C, None, True, None, None
            #return matrix_inverse_root_newton(A,int(.5*root)) #get newton -1/2 root
        else:
            print("NOT CHOL: ", root)
            return matrix_inverse_root_newton(A,root,epsilon) #standard newton -1/4 root
    ...
    def matrix_inverse_root_newton(
        ...
        """
        root*=2
        print("NEWTON", A.shape, root)
        #temp=shampoo.matrix_functions.ComputePower(A, root)
        ...
        # initialize matrices
        A_nrm = torch.linalg.norm(A_ridge).item() ###GET A_NRM VALUE
        #print('ANORM and ROOT2', A_nrm, root, root/A_nrm)
        z = (root + 1) / (2 * A_nrm)

        ### TYPE CHANGES
        root=int(root) ### NEED INT ROOT
        # z=torch.Tensor([z])
        ...
        X = z ** (-alpha) * identity
        M = z * A_ridge
        #print("herehere", type(M), type(alpha))
        error = torch.dist(M, identity, p=torch.inf)      
        # main for loop
        while error > tolerance and iteration < max_iterations:
            iteration += 1
            M_p = M.mul(torch.Tensor([alpha])).add_(identity, alpha=float(1 - alpha)) ###TENSOR ALPHA FOR MUL, FLOAT ALPHA FOR _ADD
            X = X @ M_p
            M = torch.linalg.matrix_power(M_p, root) @ M ###INT ROOT HERE
            error = torch.dist(M, identity, p=torch.inf)
            #print("ERROR", error)
        ...
        print("X: ", X)
        print("normX: ", torch.linalg.norm(X))
    ...
    match root_inv_config:
        case CholeskyConfig():
            ...
            X, _, termination_flag, _, _=_assign_function_args_from_config(
                func=matrix_inverse_root_chol,config=root_inv_config
