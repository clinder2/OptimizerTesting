...
class ExperimentalShampoo(Optimizer):
    ...
    def step(self):
        ...
        for g in self.param_groups:
            ...
            state['L'] = L + .001*torch.eye(L.shape[1],device=self.device).unsqueeze(0).repeat(L.shape[0],1,1)
            state['R'] = R + .001*torch.eye(R.shape[1],device=self.device).unsqueeze(0).repeat(R.shape[0],1,1)
            total+=L.shape[0]

            Rp, infoR = torch.linalg.cholesky_ex(state['R'],upper=True) #Cholesky decomp of R
            failedCholR = infoR.nonzero(as_tuple=True)[0].tolist()

            if len(failedCholR)>0:
                print('failedR')
                Rp[failedCholR]=ComputePower(state['R'][failedCholR], self.p//2, iter_count=50)
