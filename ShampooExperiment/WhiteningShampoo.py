...
class WhiteningShampoo(Optimizer):
    def __init__(self, groups, lr, debug=False, pure=False):
        data=dict(lr=lr)
        ...
        self.pure=pure
        self.iter=0
        self.beta2=.85 #for L, R exp. decay (.85)
        ...
        for g in self.param_groups:
            for p in g['params']:
                ...
                if self.pure:
                    ...
                    Lp=torch.linalg.inv_ex(.001*torch.eye(Lp.L.shape[0],device=self.device)+Lp.L)
                    ...
                    success=True
                    Lp=torch.linalg.cholesky_ex(.00001*torch.eye(self.state[p]['L'].shape[0],device=self.device)+self.state[p]['L'])
                    ...
                    Rp=torch.linalg.cholesky_ex(.00001*torch.eye(self.state[p]['R'].shape[0],device=self.device)+self.state[p]['R'])
...
if __name__=='__main__':
    ...
    #WS=CustomShampoo(learning_rate,params)
    ...
    while True:
        ...
        L.backward()
        print("Loss: ", L.item())
        #print(torch.linalg.norm(G,ord='fro'))
